"""
使用角色扮演提示(Role-Act)的实验脚本，对中文数据集进行测试
实现了并行处理以提高效率
"""
import json
import asyncio
...
from agents.single_prompt.role_act_prompt_agent import RoleActPromptAgent
...
async def main():
    ...
    async with AsyncApiRequester(max_tokens=8192) as api_requestor:
        ...
        # 保存JSON结果
        output_json_file = os.path.join(output_dir, "role_act_200_zh_output.json")
        ...
        # 保存CSV结果
        output_csv_file = os.path.join(output_dir, "role_act_prompt_result.csv")
